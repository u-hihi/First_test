...
-브랜치 삭제
    ...
    위 2개는 내 컴퓨터에서 삭제할 때의 경우이고

    명령어: git push origin --delete test

    다음은 GitHub에서 삭제할 때의 경우입니다.
...
이런 식으로 나오는데 원하는 내용만 남기고 <,=,> 표시는 다 지워야됩니다.

code 저장 후 add → commit → push 순으로 저장하면 됩니다.

### .gitignore
GitHub에 올리면 안 되는 파일을 자동으로 제외하는 파일입니다.

예를 들어 비밀번호, API키, 개인정보 등이 있습니다.
